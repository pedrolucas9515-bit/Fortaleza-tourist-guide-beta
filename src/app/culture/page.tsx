    ...
    <div className="min-h-screen bg-[#0f1315] pb-24">
      <header className="px-6 pt-12 pb-6 hud-gradient sticky top-0 z-20">
        ...
        <h1 className="font-headline text-3xl text-white flex items-center gap-3">
          {t.cultureEvents} <Music className="w-6 h-6 text-purple-400" />
        </h1>
      </header>
      ...
      <div className="px-6 space-y-6">
        {cultureSpots.map((spot) => (
          <Link key={spot.id} href={`/attraction/${spot.id}`}>
            <Card className="overflow-hidden glass border-white/5 rounded-[2rem] group mb-6">
              ...
              <div className="p-6">
                <h3 className="font-headline text-2xl text-white mb-2">{spot.title[language]}</h3>
        ...
        <Card className="glass border-primary/20 p-6 rounded-[2rem] bg-primary/5">
           ...
           <p className="text-xs text-white/80 leading-relaxed italic">
             {language === 'pt' ? 'Fique atento aos eventos culturais que ocorrem quase diariamente no calçadão da Beira Mar, incluindo shows de humor e música ao vivo.' : 
              ...
              language === 'es' ? 'Esté atento a los eventos culturales que ocurren casi a diario en el paseo de Beira Mar, incluidos espectáculos de humor y música en vivo.' :
