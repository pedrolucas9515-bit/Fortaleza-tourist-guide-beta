...
export default function FactsPage() {
  ...
  const facts = [
    { icon: ThermometerSun, text: t.fact1, color: 'text-orange-400' },
    { icon: UtensilsCrossed, text: t.fact2, color: 'text-red-400' },
    { icon: Laugh, text: t.fact3, color: 'text-yellow-400' }
  ];
    ...
    <div className="min-h-screen bg-[#0f1315] pb-24">
      <header className="px-6 pt-12 pb-6 hud-gradient sticky top-0 z-20">
        ...
        <h1 className="font-headline text-3xl text-white flex items-center gap-3">
          {t.facts} <Sparkles className="w-6 h-6 text-yellow-400 animate-pulse" />
        </h1>
      </header>

      <div className="px-6 space-y-6">
        {facts.map((fact, idx) => (
          <Card key={idx} className="glass border-white/5 p-8 rounded-[2.5rem] relative overflow-hidden group">
            <div className="absolute -right-8 -bottom-8 opacity-5 group-hover:opacity-10 transition-opacity">
              <fact.icon className={`w-32 h-32 ${fact.color}`} />
            </div>
            
            <div className={`p-4 rounded-3xl bg-white/5 inline-block mb-6`}>
